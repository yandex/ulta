...
install_jmeter() {
    ...
    # ABCADVISOR-68068: tika-parsers — точка входа XXE (XFA в PDF) и нужен только GUI-фиче
    # Document parsing. tika-core не трогать: без него jmeter не грузит .jmx с HTTP-сэмплером.
    rm -f ${JMETER_HOME}/lib/tika-parsers-*.jar
